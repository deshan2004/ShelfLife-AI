    ...
    <div className="admin-inventory">
      <div className="admin-page-header">
        ...
        <div className="admin-inventory-stats">
          <div className="admin-stat-chip"><i className="fas fa-box"></i> <span>{stats.total} Products</span></div>
          <div className="admin-stat-chip warning"><i className="fas fa-clock"></i> <span>{stats.expiring} Expiring</span></div>
          <div className="admin-stat-chip critical"><i className="fas fa-exclamation-triangle"></i> <span>{stats.critical} Critical</span></div>
          <div className="admin-stat-chip danger"><i className="fas fa-skull"></i> <span>{stats.expired} Expired</span></div>
          <div className="admin-stat-chip success"><i className="fas fa-chart-line"></i> <span>LKR {stats.totalValue.toLocaleString()}</span></div>
        </div>
      </div>
      ...
      <div className="admin-filters-bar">
        <div className="admin-search-box">
          ...
          <input type="text" placeholder="Search products or store owners..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
        </div>
              ...
              <tr key={`${product.ownerId}-${product.id}-${index}`} className={product.daysLeft <= 0 ? 'expired-row' : product.daysLeft <= 3 ? 'critical-row' : product.daysLeft <= 7 ? 'warning-row' : ''}>
                ...
                <td>
                  <div className="admin-store-info">
                    <div className="admin-store-avatar">{product.ownerAvatar || 'U'}</div>
                    <div>
