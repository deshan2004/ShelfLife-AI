...
if (process.env.FIREBASE_SERVICE_ACCOUNT) {
  ...
  if (fs.existsSync(serviceAccountPath)) {
    ...
    console.error('Please set FIREBASE_SERVICE_ACCOUNT env var.');
  }
}
...
app.post('/api/suppliers/:userId/add', async (req, res) => {
  try {
    ...
    if (!supplier.name || supplier.name.trim().length < 2) {
      return res.status(400).json({ error: 'Supplier name is required (minimum 2 characters)' });
    }
...
app.get('/', (req, res) => {
  res.json({
    name: 'ShelfLife AI Backend (Vercel)',
    version: '2.0.0',
    status: 'Online',
    firebase: db ? 'connected' : 'disconnected',
    endpoints: {
