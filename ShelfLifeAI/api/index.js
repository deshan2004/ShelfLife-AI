...
let serviceAccount;
try {
  // Production: environment variable එකෙන්
  if (process.env.FIREBASE_SERVICE_ACCOUNT) {
    serviceAccount = JSON.parse(process.env.FIREBASE_SERVICE_ACCOUNT);
  } else {
    // Development: file එකෙන්
...
// ============================================================
// 5. EMAIL SEND FUNCTION
// ============================================================
async function sendSupplierEmail(supplierEmail, supplierName, product, notificationType, customMessage) {
  if (!emailTransporter) {
    console.log('📧 Email would be sent (simulated):', {
      to: supplierEmail,
      subject: `${notificationType === 'low_stock' ? 'Low Stock' : 'Near Expiry'} Alert: ${product.name}`,
      message: customMessage || 'Email sending disabled - check EMAIL_USER and EMAIL_PASS'
    });
    return { success: true, messageId: 'simulated_' + Date.now() };
  }

  try {
    const isLowStock = notificationType === 'low_stock';
    const subject = isLowStock 
      ? `📦 Low Stock Alert: ${product.name}` 
      : `⏰ Near Expiry Alert: ${product.name}`;
    
    const message = customMessage || (isLowStock
      ? `Dear ${supplierName},\n\nWe need to reorder ${product.name}.\nCurrent stock: ${product.stock} units (Threshold: ${product.lowStockThreshold || 10}).\n\nPlease send ${(product.lowStockThreshold || 10) - product.stock + 10} units urgently.\n\nThank you,\nShelfLife AI`
      : `Dear ${supplierName},\n\n${product.name} is nearing expiry (${product.daysLeft} days left).\nWe request you to arrange a return or exchange.\n\nThank you,\nShelfLife AI`);
...
app.get('/', (req, res) => {
  res.json({
    message: 'ShelfLife AI Backend is running on Vercel!',
    status: 'Online',
...
// ============================================================
// 9. SUPPLIER NOTIFICATION - SEND EMAIL
// ============================================================
app.post('/api/suppliers/notify', async (req, res) => {
  try {
    const { userId, productId, notificationType, customMessage } = req.body;
    ...
    const suppliersDoc = await db.collection('suppliers').doc(userId).get();
    if (!suppliersDoc.exists) {
      return res.status(404).json({ error: 'No suppliers found for this user' });
    }
    
    const suppliers = suppliersDoc.data().suppliers || [];
    const supplier = suppliers.find(s => s.name === product.supplier);
    
    if (!supplier) {
      return res.status(404).json({ 
        error: `Supplier "${product.supplier}" not found in your supplier list.` 
      });
    }
    
    if (!supplier.email) {
      return res.status(400).json({ 
        error: `Supplier "${supplier.name}" has no email address. Please update supplier profile.` 
      });
    }
    
    const type = notificationType || (product.stock <= product.lowStockThreshold ? 'low_stock' : 'near_expiry');
